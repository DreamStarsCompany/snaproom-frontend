...
export default function AdminUser() {
  ...
  const [searchTerm, setSearchTerm] = useState('');
  const [roleFilter, setRoleFilter] = useState('');
  const [mode, setMode] = useState('user'); // user | verify
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', width: 300, bgcolor: 'white', borderRadius: '999px', px: 2, py: '6px', boxShadow: '0 0 0 1px #ccc' }}>
                ...
                <InputBase
                  ...
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  sx={{ flex: 1, color: 'gray', fontSize: 16 }}
                  disableUnderline
                />
              </Box>

              {/* Role filter (only for user mode) */}
              {mode === 'user' && (
                <Box sx={{ width: 180, bgcolor: 'white', borderRadius: '999px', boxShadow: '0 0 0 1px #ccc', px: 2 }}>
                  <TextField
                    select
                    variant="standard"
                    value={roleFilter}
                    onChange={(e) => {
                      console.log('RoleFilter Value:', e.target.value);
              ...
              {/* Mode switch */}
              <Box sx={{ width: 200, bgcolor: 'white', borderRadius: '999px', boxShadow: '0 0 0 1px #ccc', px: 2 }}>
                <TextField
                  select
                  variant="standard"
                  value={mode}
